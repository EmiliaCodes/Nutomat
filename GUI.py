...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.button_generate = customtkinter.CTkButton(master=self.frame_top, text="GENERUJ",
                                                       ...
                                                       fg_color="#11B384",
                                                       hover_color="#92374d", text_font=("TkDefaultFont", 10, 'bold')
                                                       )
        ...
        self.label_key_signature = customtkinter.CTkLabel(master=self.frame_left, text="Tonacja dyktanda", width=100,
                                                          anchor="w")
        self.label_key_signature.grid(row=1, column=0, pady=10, padx=(0, 0))

        self.option_key_signature = customtkinter.CTkOptionMenu(master=self.frame_left,
                                                                values=["C", "F", "G", "B", "D"])
        self.option_key_signature.grid(row=1, column=1, pady=10, padx=20, sticky="w")

        self.label_number_of_bars = customtkinter.CTkLabel(master=self.frame_left,
                                                           width=100,
                                                           anchor="w",
                                                           text="Ilość taktów")
        self.label_number_of_bars.grid(row=2, column=0, pady=10, padx=(0, 0))

        self.option_number_of_bars = customtkinter.CTkOptionMenu(master=self.frame_left, values=["2", "4", "8", "16"])
        self.option_number_of_bars.grid(row=2, column=1, pady=10, padx=20, sticky="w")

        self.label_tempo = customtkinter.CTkLabel(master=self.frame_left, text="Tempo dyktanda", width=100, anchor="w")
        self.label_tempo.grid(row=3, column=0, pady=10, padx=(0, 0))

        self.option_tempo = customtkinter.CTkOptionMenu(master=self.frame_left,
                                                        values=["szybkie", "umiarkowane", "wolne"])
        self.option_tempo.grid(row=3, column=1, pady=10, padx=20, sticky="w")
        ...
        self.frame_right.grid_rowconfigure(4, weight=10, minsize=5)  # empty row as spacing
    ...
    def generate(self):
        # TODO loading screen or at least loading cursor
        ...
        # Should temperature be somewhere else?
        temperature = 0.5
        ...
        App.my_engine.save_file(pitch_final_prediction, rhythm_final_prediction, my_path, key_signature,
                                tempo, write_midi, write_empty, hints)
