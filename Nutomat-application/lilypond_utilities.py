...
def music21_to_lily(pitch_final_prediction, rhythm_final_prediction):
    ...
    for i, (note_pitch, note_rhythm) in enumerate(zip(pitch_final_prediction, rhythm_final_prediction)):
        ...
        if note_rhythm == Fraction(1, 3):
            tuplet_number += 1

        this_pitch = melody_translator(note_pitch)
        this_rhythm = rhythm_translator(note_rhythm, this_pitch, tuplet_number)
        lily_notation.append(this_rhythm)

        if tuplet_number == 3:  # if this was the third of the triplet
            tuplet_number = 0
...
def rhythm_translator(music21_notation, pitch, tuplet_number):
    ...
    switcher = {
        ...
        Fraction(1, 3): "\\tuplet 3/2 {{ {pitch} 8 }}".format(pitch=pitch),  # triola ósemkowa
        ...
        1.25: "{pitch} 4~ {pitch} 16".format(pitch=pitch),
        1.5: "{pitch} 4.".format(pitch=pitch),  # ćwierćnuta z kropką
        2.0: "{pitch} 2".format(pitch=pitch),  # półnuta
        2.5: "{pitch} 2~ {pitch} 8".format(pitch=pitch),
        2.75: "{pitch} 2.".format(pitch=pitch),  # półnuta z kropką
        3.0: "{pitch} 2.".format(pitch=pitch),
        3.75: "{pitch} 2.~ {pitch} 8.".format(pitch=pitch),
        4.0: "{pitch} 1".format(pitch=pitch)
    }

    if tuplet_number == 2:
        add_before = "\\undo \\omit TupletNumber"
        add_after = "\\omit TupletNumber"
        note_text = switcher.get(music21_notation)
        return add_before + note_text + add_after
